...
If I ever decide to release this extension on the gallery, this will be replaced with an actual docs page.`);
    ...
    setFogAtt({att, v}) {
      ...
      {
        case "near":
          this.scene.fog.near = args["v"];
          break;
        case "far":
          this.scene.fog.far = args["v"];
          break;
      }
      this.updateRenderer();
    }
    ...
    setLightDir({name, DIRECTION, DEGREES}) {
      ...
      if (light instanceof THREE.SpotLight) {
          ...
          switch (DIRECTION) {
              case "y":
              case "angle":
                  light.target.position.set(light.target.position.x, -DEGREES, DEGREES);
                  break;
              case "x":
              case "aim":
                  light.target.position.set(-DEGREES, light.target.position.y,  DEGREES);
                  break;
              case "z":
              case "roll":
                  light.target.position.set(-DEGREES, DEGREES,  light.target.position.z);
                  break;
              default:
                  console.warn(`Direction '${DIRECTION}' is not valid. Use 'x', 'y', or 'z'.`);
                  return;
          }
    ...
    deleteLight({name}) {
      this.scene.remove(LIGHTS[name]); // Safely remove the light from the scene first
      delete LIGHTS[name];
      this.updateRenderer();
    }
    deleteLights() {
      var chi = null
      for(const obj in this.scene.children)
      {
        chi = this.scene.children[obj];
        if(chi.isLight)
        {
          this.scene.remove(chi);
        }
      }
      LIGHTS = {};
      this.updateRenderer();
    }
    getLightPos({name, x, y, z}) {
      let l = LIGHTS[name]
      switch(pos){
        case "x": {
          return l.position["x"];
